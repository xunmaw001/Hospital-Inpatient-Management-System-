...
<script>
    ...
    function setDataRow(item, number) {
        ...
        if(item.yonghuName == null){
            yonghuNameCell.innerHTML = "护士暂未执行";
        }else{
        ...
        row.appendChild(yizhuTypesCell);
            ...
            //医嘱内容
        var zhuyuanfeiyongContentCell = document.createElement('td');
        //如果图文详情包含图片
        if (item.zhuyuanfeiyongContent && item.zhuyuanfeiyongContent.indexOf('img') != -1) {
            //暂时只考虑图片+文字 和 文字+图片的情况
            var beginIndex = item.zhuyuanfeiyongContent.indexOf('<img');
            var endIndex = item.zhuyuanfeiyongContent.indexOf('>');
                zhuyuanfeiyongContentCell.innerHTML = item.zhuyuanfeiyongContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
        } else {
            if (item.zhuyuanfeiyongContent != null && item.zhuyuanfeiyongContent != "" && item.zhuyuanfeiyongContent.length >= 11) {
                    zhuyuanfeiyongContentCell.innerHTML = item.zhuyuanfeiyongContent.substring(0, 10) + "...";
            } else {
                    zhuyuanfeiyongContentCell.innerHTML = item.zhuyuanfeiyongContent;
            }
        }
        row.appendChild(zhuyuanfeiyongContentCell);
        ...
        if(item.yonghuName == null){
            ...
            var detailAttr = "zhixing(" + item.id + ')';
            detailBtn.setAttribute("type", "button");
            detailBtn.setAttribute("class", "btn btn-info btn-sm 执行");
            detailBtn.setAttribute("onclick", detailAttr);
            detailBtn.innerHTML = "执行"
    ...
    // 删除
    function zhixing(id) {
        var mymessage = confirm("真的要执行这个医嘱吗？");
        if (mymessage == true) {
            http("yizhu/zhixing?ids="+id, "get", {}, (res) => {
                if(res.code == 0){
                    getDataList();
                    alert('执行成功');
                }

    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑病人管理</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑病人管理</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">病人管理信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="bingrenAge" name="bingrenAge" class="form-control"
                                               onchange="bingrenAgeChickValue(this)"  placeholder="年龄">
                                    </div>

                                    <div class="form-group col-md-6">
                                        <label>家属联系方式</label>
                                        <input id="bingrenPhone" name="bingrenPhone" class="form-control"
                                               onchange="bingrenPhoneChickValue(this)"  placeholder="家属联系方式">
                                    </div>
                                <input id="yishengId" name="yishengId" type="hidden">
                                    <div class="form-group col-md-6">
                                        <label>患病类型</label>
                                        <select id="huanbingTypesSelect" name="huanbingTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group  col-md-6">
                                        <label>患病详情</label>
                                        <input id="bingrenContentupload" name="file" type="file">
                                        <script id="bingrenContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var bingrenContentUe = UE.getEditor('bingrenContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="bingrenContent" id="bingrenContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#bingrenContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('bingrenContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            let data = {};
            getContent();
            if(window.sessionStorage.getItem('role') != "医生"){

               if($("#yishengSelect") !=null){
                   var yishengId = $("#yishengSelect").val();
            ...
            data['yishengId'] = $("#yishengSelect").val();
            ...
            httpJson("bingren/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addbingren');
    ...
    // 查询列表
        ...
        function yishengSelect() {
            ...
            http("yisheng/page?page=1&limit=100&sort=&order=", "GET", {}, (res) => {
                ...
                yishengSelectOne(window.sessionStorage.getItem("userId"));
                }
            });
        }
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationHuanbingtypesSelect(){
            var huanbingTypesSelect = document.getElementById('huanbingTypesSelect');
            if(huanbingTypesSelect != null && huanbingTypesOptions != null  && huanbingTypesOptions.length > 0 ){
                for (var i = 0; i < huanbingTypesOptions.length; i++) {
                    huanbingTypesSelect.add(new Option(huanbingTypesOptions[i].indexName,huanbingTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(huanbingTypesSelect != null && huanbingTypesOptions != null  && huanbingTypesOptions.length > 0 ) {
            for (var i = 0; i < huanbingTypesOptions.length; i++) {
                if (huanbingTypesOptions[i].codeIndex == ruleForm.huanbingTypes) {//下拉框value对比,如果一致就赋值汉字
                        huanbingTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.bingrenContent != null && ruleForm.bingrenContent != 'null' && ruleForm.bingrenContent != '' && $("#bingrenContentupload").length>0) {

            var bingrenContentUeditor = UE.getEditor('bingrenContentEditor');
            bingrenContentUeditor.ready(function () {
                var mes = '' + ruleForm.bingrenContent;
                bingrenContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#bingrenContentupload").length>0) {
            var bingrenContentEditor = UE.getEditor('bingrenContentEditor');
            if (bingrenContentEditor.hasContents()) {
                $('#bingrenContent-input').attr('value', bingrenContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function bingrenAgeChickValue(e){
        ...
        function bingrenPhoneChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                bingrenName: "required",
                sexTypes: "required",
                bingrenAge: "required",
                bingrenPhone: "required",
                yishengId: "required",
                huanbingTypes: "required",
                bingrenContent: "required",
            },
            messages: {
                bingrenName: "病人名称不能为空",
                sexTypes: "性别不能为空",
                bingrenAge: "年龄不能为空",
                bingrenPhone: "家属联系方式不能为空",
                yishengId: "检查医生不能为空",
                huanbingTypes: "患病类型不能为空",
                bingrenContent: "患病详情不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addbingren = window.sessionStorage.getItem("addbingren");
        if (addbingren != null && addbingren != "" && addbingren != "null") {
            window.sessionStorage.removeItem('addbingren');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("bingren/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationHuanbingtypesSelect();
            <!--  初始化级联表的下拉框  -->
